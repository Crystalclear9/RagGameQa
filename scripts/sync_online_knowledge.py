...
def parse_args():
    ...
    parser.add_argument("--include-crawler", action="store_true", help="Also try crawler sources")
    parser.add_argument("--crawler-max-pages", type=int, default=3, help="Crawler max pages")
...
async def main_async() -> int:
    ...
    result = await service.sync(
        ...
        include_crawler=args.include_crawler,
        crawler_max_pages=max(1, min(args.crawler_max_pages, 20)),
    )
    print("=" * 64)
    ...
    print(f"[INFO] crawler_docs: {result.get('crawler_docs')}")
