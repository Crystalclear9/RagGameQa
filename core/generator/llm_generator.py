...
class LLMGenerator:
    ...
    def __init__(self, game_id: str):
        ...
        # 读取默认RAG/生成模型配置
        self.model_name = ModelConfig.RAG_MODELS.get(settings.DEFAULT_MODEL, {}).get(
            "model_name", "gpt-4"
        )
        self.temperature = ModelConfig.RAG_MODELS.get(settings.DEFAULT_MODEL, {}).get(
            "temperature", 0.7
        )
        self.max_tokens = min(
            int(ModelConfig.RAG_MODELS.get(settings.DEFAULT_MODEL, {}).get("max_tokens", 2000)),
            2000,
        )
